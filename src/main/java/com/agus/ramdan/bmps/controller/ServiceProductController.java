...
import com.agus.ramdan.bmps.repository.ServiceProductRepository;
...
@RestController
@RequestMapping("/api/bmps/service_product")
public class ServiceProductController {
    @Autowired
    ServiceProductRepository repository;

    @GetMapping("")
    @Operation(summary = "Get All")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "successful operation",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = ServiceProduct.class)))),
    ...
    public ResponseEntity<List<ServiceProduct>> getAll(
        ...
        val builder = new BaseSpecificationsBuilder<ServiceProduct>();
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "successful operation",content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ServiceProduct.class)), })
    })
    public ResponseEntity<ServiceProduct> getById(@PathVariable("id") long id)
    ...
    public ResponseEntity<ServiceProduct> postCreate(@RequestBody ServiceProduct new_data) {
    ...
    public ResponseEntity<ServiceProduct> putUpdate(
            @PathVariable("id") long id,
            @RequestBody ServiceProduct update_data)
            throws ResourceNotFoundException {
        var data = ChekUtils.getOrThrow(repository.findById(id),()-> "Data not found for this id :: " + id);
