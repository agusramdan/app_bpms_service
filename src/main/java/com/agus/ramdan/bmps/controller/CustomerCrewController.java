...
@RestController
@RequestMapping("/api/bmps/customer_crew")
public class CustomerCrewController {
    @Autowired
    CustomerCrewRepository repository;

    @GetMapping("")
    @Operation(summary = "Get All")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "successful operation",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = CustomerCrew.class)))),
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "successful operation",content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomerCrew.class)), })
    })
    public ResponseEntity<CustomerCrew> getById(@PathVariable("id") long id)
    ...
    public ResponseEntity<CustomerCrew> postCreate(@RequestBody CustomerCrew new_data) {
    ...
    public ResponseEntity<CustomerCrew> putUpdate(
            @PathVariable("id") long id,
            @RequestBody CustomerCrew update_data)
            throws ResourceNotFoundException {
        var data = ChekUtils.getOrThrow(repository.findById(id),()-> "Data not found for this id :: " + id);
