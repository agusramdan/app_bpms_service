...
import com.agus.ramdan.bmps.repository.ServiceLocationRepository;
...
@RestController
@RequestMapping("/api/bmps/service_location")
public class ServiceLocationController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "successful operation",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = ServiceLocation.class)))),
    ...
    public ResponseEntity<List<ServiceLocation>> getAll(
            @RequestParam(value = "offset", required = false, defaultValue = "0") int offset,
            @RequestParam(value = "limit", required = false, defaultValue = "25") int limit,
            @RequestParam(value = "search", required = false) String search
    ) {
        var pageable = new OffsetBasedPageRequest(offset,limit);
        val builder = new BaseSpecificationsBuilder<ServiceLocation>();
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "successful operation",content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ServiceLocation.class)), })
    })
    public ResponseEntity<ServiceLocation> getById(@PathVariable("id") long id)
    ...
    public ResponseEntity<ServiceLocation> postCreate(@RequestBody ServiceLocation new_data) {
    ...
    public ResponseEntity<ServiceLocation> putUpdate(
            @PathVariable("id") long id,
            @RequestBody ServiceLocation update_data)
            throws ResourceNotFoundException {
        var data = ChekUtils.getOrThrow(repository.findById(id),()-> "Data not found for this id :: " + id);
