...
import com.agus.ramdan.bmps.repository.VendorCrewRepository;
...
@RestController
@RequestMapping("/api/bmps/vendor_crew")
public class VendorCrewController {
    @Autowired
    VendorCrewRepository repository;

    @GetMapping("")
    @Operation(summary = "Get All")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "successful operation",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = VendorCrew.class)))),
    ...
    public ResponseEntity<List<VendorCrew>> getAll(
            @RequestParam(value = "offset", required = false, defaultValue = "0") int offset,
            @RequestParam(value = "limit", required = false, defaultValue = "25") int limit,
            @RequestParam(value = "search", required = false) String search,
            @RequestParam(value = "ids", required = false) String ids
    ) {
        val builder = new BaseSpecificationsBuilder<VendorCrew>();
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "successful operation",content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = VendorCrew.class)), })
    })
    public ResponseEntity<VendorCrew> getById(@PathVariable("id") long id)
    ...
    public ResponseEntity<VendorCrew> postCreate(@RequestBody VendorCrew new_data) {
    ...
    public ResponseEntity<VendorCrew> putUpdate(
            @PathVariable("id") long id,
            @RequestBody VendorCrew update_data)
            throws ResourceNotFoundException {
        var data = ChekUtils.getOrThrow(repository.findById(id),()-> "Data not found for this id :: " + id);
